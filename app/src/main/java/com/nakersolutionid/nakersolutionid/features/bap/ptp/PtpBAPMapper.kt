...
fun PtpBAPReport.toInspectionWithDetailsDomain(
    ...
    val inspectionDomain = InspectionDomain(
        ...
        subInspectionType = if (this.equipmentType.contains(
                "diesel",
                ignoreCase = true
            )
        ) SubInspectionType.Motor_Diesel else SubInspectionType.Machine,
    ...
    checkItems.add(
        InspectionCheckItemDomain(
            ...
            category = PtpBAPCategory.TECHNICAL_DATA,
            itemName = "Perlengkapan pengaman terpasang",
            status = this.technicalData.areSafetyFeaturesInstalled
        )
    )
...
private fun mapTechnicalDataToDomain(
    uiState: PtpBAPTechnicalData,
    inspectionId: Long
): List<InspectionTestResultDomain> {
    val cat = PtpBAPCategory.TECHNICAL_DATA
    return listOf(
        InspectionTestResultDomain(
            inspectionId = inspectionId,
            testName = "Negara Pembuat",
            result = uiState.manufactureCountry,
            notes = cat
        ),
        InspectionTestResultDomain(
            inspectionId = inspectionId,
            testName = "Keterangan Kapasitas",
            result = uiState.capacityDescription,
            notes = cat
        ),
        InspectionTestResultDomain(
            inspectionId = inspectionId,
            testName = "Daya Motor Penggerak (kW)",
            result = uiState.driveMotorPowerKw,
            notes = cat
        ),
        InspectionTestResultDomain(
            inspectionId = inspectionId,
            testName = "Spesifikasi Khusus",
            result = uiState.specialSpecification,
            notes = cat
        ),
        InspectionTestResultDomain(
            inspectionId = inspectionId,
            testName = "Keterangan Dimensi",
            result = uiState.dimensionsDescription,
            notes = cat
        ),
        InspectionTestResultDomain(
            inspectionId = inspectionId,
            testName = "Putaran (RPM)",
            result = uiState.rotationRpm,
            notes = cat
        ),
        InspectionTestResultDomain(
            inspectionId = inspectionId,
            testName = "Frekuensi (Hz)",
            result = uiState.frequencyHz,
            notes = cat
        ),
        InspectionTestResultDomain(
            inspectionId = inspectionId,
            testName = "Arus (Ampere)",
            result = uiState.currentAmperes,
            notes = cat
        ),
        InspectionTestResultDomain(
            inspectionId = inspectionId,
            testName = "Berat Mesin (Kg)",
            result = uiState.machineWeightKg,
            notes = cat
        )
    )
}

private fun mapVisualInspectionToDomain(
    uiState: PtpBAPVisualInspection,
    inspectionId: Long
): List<InspectionCheckItemDomain> {
    return mutableListOf<InspectionCheckItemDomain>().apply {
        add(
            InspectionCheckItemDomain(
                inspectionId = inspectionId,
                category = PtpBAPCategory.VISUAL_INSPECTION,
                itemName = "Kondisi mesin baik",
                status = uiState.isMachineGoodCondition
            )
        )
        add(
            InspectionCheckItemDomain(
                inspectionId = inspectionId,
                category = PtpBAPCategory.VISUAL_INSPECTION,
                itemName = "Indikator kelistrikan baik",
                status = uiState.areElectricalIndicatorsGood
            )
        )
        add(
            InspectionCheckItemDomain(
                inspectionId = inspectionId,
                category = PtpBAPCategory.VISUAL_INSPECTION,
                itemName = "Pembumian terpasang",
                status = uiState.isGroundingInstalled
            )
        )
        add(
            InspectionCheckItemDomain(
                inspectionId = inspectionId,
                category = PtpBAPCategory.VISUAL_INSPECTION,
                itemName = "Kondisi baterai baik",
                status = uiState.isBatteryGoodCondition
            )
        )
        add(
            InspectionCheckItemDomain(
                inspectionId = inspectionId,
                category = PtpBAPCategory.VISUAL_INSPECTION,
                itemName = "Ada kebocoran pelumasan",
                status = uiState.hasLubricationLeak
            )
        )
        add(
            InspectionCheckItemDomain(
                inspectionId = inspectionId,
                category = PtpBAPCategory.VISUAL_INSPECTION,
                itemName = "Kondisi pondasi baik",
                status = uiState.isFoundationGoodCondition
            )
        )
        add(
            InspectionCheckItemDomain(
                inspectionId = inspectionId,
                category = PtpBAPCategory.VISUAL_INSPECTION,
                itemName = "Ada kebocoran hidrolik",
                status = uiState.hasHydraulicLeak
            )
        )
        add(
            InspectionCheckItemDomain(
                inspectionId = inspectionId,
                category = PtpBAPCategory.VISUAL_PPE,
                itemName = "APAR tersedia",
                status = uiState.personalProtectiveEquipment.isAparAvailable
            )
        )
        add(
            InspectionCheckItemDomain(
                inspectionId = inspectionId,
                category = PtpBAPCategory.VISUAL_PPE,
                itemName = "APD tersedia",
                status = uiState.personalProtectiveEquipment.isPpeAvailable
            )
        )
    }
}

private fun mapTestingToDomain(
    uiState: PtpBAPTesting,
    inspectionId: Long
): List<InspectionCheckItemDomain> {
    ...
    return listOf(
        InspectionCheckItemDomain(
            inspectionId = inspectionId,
            category = cat,
            itemName = "Penerangan memenuhi syarat",
            status = uiState.isLightingCompliant
        ),
        InspectionCheckItemDomain(
            inspectionId = inspectionId,
            category = cat,
            itemName = "Tingkat kebisingan memenuhi syarat",
            status = uiState.isNoiseLevelCompliant
        ),
        InspectionCheckItemDomain(
            inspectionId = inspectionId,
            category = cat,
            itemName = "Tombol darurat berfungsi",
            status = uiState.isEmergencyStopFunctional
        ),
        InspectionCheckItemDomain(
            inspectionId = inspectionId,
            category = cat,
            itemName = "Mesin berfungsi baik",
            status = uiState.isMachineFunctional
        ),
        InspectionCheckItemDomain(
            inspectionId = inspectionId,
            category = cat,
            itemName = "Tingkat getaran memenuhi syarat",
            status = uiState.isVibrationLevelCompliant
        ),
        InspectionCheckItemDomain(
            inspectionId = inspectionId,
            category = cat,
            itemName = "Tahanan isolasi baik",
            status = uiState.isInsulationResistanceOk
        ),
        InspectionCheckItemDomain(
            inspectionId = inspectionId,
            category = cat,
            itemName = "Putaran poros memenuhi syarat",
            status = uiState.isShaftRotationCompliant
        ),
        InspectionCheckItemDomain(
            inspectionId = inspectionId,
            category = cat,
            itemName = "Tahanan pembumian memenuhi syarat",
            status = uiState.isGroundingResistanceCompliant
        ),
        InspectionCheckItemDomain(
            inspectionId = inspectionId,
            category = cat,
            itemName = "Hasil NDT Las baik",
            status = uiState.isNdtWeldTestOk
        ),
        InspectionCheckItemDomain(
            inspectionId = inspectionId,
            category = cat,
            itemName = "Tegangan antar fasa normal",
            status = uiState.isVoltageBetweenPhasesNormal
        ),
        InspectionCheckItemDomain(
            inspectionId = inspectionId,
            category = cat,
            itemName = "Beban antar fasa seimbang",
            status = uiState.isPhaseLoadBalanced
        )
    )
}
...
fun InspectionWithDetailsDomain.toPtpBAPReport(): PtpBAPReport {
    fun findBoolItem(category: String, itemName: String): Boolean {
        return this.checkItems.find { it.category == category && it.itemName == itemName }?.status
            ?: false
    }
    ...
    val technicalData = PtpBAPTechnicalData(
        ...
        areSafetyFeaturesInstalled = findBoolItem(
            PtpBAPCategory.TECHNICAL_DATA,
            "Perlengkapan pengaman terpasang"
        )
    )

    val visualInspection = PtpBAPVisualInspection(
        isMachineGoodCondition = findBoolItem(
            PtpBAPCategory.VISUAL_INSPECTION,
            "Kondisi mesin baik"
        ),
        areElectricalIndicatorsGood = findBoolItem(
            PtpBAPCategory.VISUAL_INSPECTION,
            "Indikator kelistrikan baik"
        ),
        isGroundingInstalled = findBoolItem(
            PtpBAPCategory.VISUAL_INSPECTION,
            "Pembumian terpasang"
        ),
        isBatteryGoodCondition = findBoolItem(
            PtpBAPCategory.VISUAL_INSPECTION,
            "Kondisi baterai baik"
        ),
        hasLubricationLeak = findBoolItem(
            PtpBAPCategory.VISUAL_INSPECTION,
            "Ada kebocoran pelumasan"
        ),
        isFoundationGoodCondition = findBoolItem(
            PtpBAPCategory.VISUAL_INSPECTION,
            "Kondisi pondasi baik"
        ),
    ...
    val testing = PtpBAPTesting(
        ...
        isNoiseLevelCompliant = findBoolItem(
            PtpBAPCategory.TESTING,
            "Tingkat kebisingan memenuhi syarat"
        ),
        isEmergencyStopFunctional = findBoolItem(
            PtpBAPCategory.TESTING,
            "Tombol darurat berfungsi"
        ),
        isMachineFunctional = findBoolItem(PtpBAPCategory.TESTING, "Mesin berfungsi baik"),
        isVibrationLevelCompliant = findBoolItem(
            PtpBAPCategory.TESTING,
            "Tingkat getaran memenuhi syarat"
        ),
        isInsulationResistanceOk = findBoolItem(PtpBAPCategory.TESTING, "Tahanan isolasi baik"),
        isShaftRotationCompliant = findBoolItem(
            PtpBAPCategory.TESTING,
            "Putaran poros memenuhi syarat"
        ),
        isGroundingResistanceCompliant = findBoolItem(
            PtpBAPCategory.TESTING,
            "Tahanan pembumian memenuhi syarat"
        ),
        isNdtWeldTestOk = findBoolItem(PtpBAPCategory.TESTING, "Hasil NDT Las baik"),
        isVoltageBetweenPhasesNormal = findBoolItem(
            PtpBAPCategory.TESTING,
            "Tegangan antar fasa normal"
        ),
...
private fun InspectionWithDetailsDomain.toPtpBAPReportFromLaporan(): PtpBAPReport {
    ...
    fun getCheckItemStatus(
        category: String,
        itemName: String,
        defaultValue: Boolean = false
    ): Boolean {
        return checkItemsByCategory[category]?.find { it.itemName == itemName }?.status
            ?: defaultValue
    }
    ...
    val visualInspection = PtpBAPVisualInspection(
        isMachineGoodCondition = getCheckItemStatus(
            "visual_inspection",
            "mainFrameCondition"
        ) || getCheckItemStatus("visual_inspection", "baseConstructionFoundation"),
        areElectricalIndicatorsGood = getCheckItemStatus(
            "visual_inspection",
            "displayCondition"
        ...
        isFoundationGoodCondition = getCheckItemStatus(
            "visual_inspection",
            "foundationCondition"
        ) || getCheckItemStatus("visual_inspection", "baseConstructionFoundation"),
        isBatteryGoodCondition = if (inspection.subInspectionType == SubInspectionType.Motor_Diesel) getCheckItemStatus(
            "visual_inspection",
            "startingSystemBatteryPoles"
        ) else false,
        hasLubricationLeak = if (inspection.subInspectionType == SubInspectionType.Motor_Diesel) !getCheckItemStatus(
            "visual_inspection",
            "lubeSystemPacking",
            true
        ) else false,
        hasHydraulicLeak = if (inspection.subInspectionType == SubInspectionType.Machine) !getCheckItemStatus(
            "visual_inspection",
            "hydraulicHoseCondition",
            true
        ) else false,
        personalProtectiveEquipment = PtpBAPPersonalProtectiveEquipment(
            isAparAvailable = false,
            isPpeAvailable = false
        ) // Not available from Laporan
    )

    // --- Test Results: Functional Testing ---
    val testing = PtpBAPTesting(
        isLightingCompliant = ((getCheckItemValue(
            "testing",
            "ndt_lighting"
        ).takeIf { it.isNotEmpty() }?.split("#")?.firstOrNull() ?: getCheckItemValue(
            "lighting_measurement",
            "pointA_result"
        )).toDoubleOrNull() ?: 0.0) > 0.0,
        isNoiseLevelCompliant = ((getCheckItemValue(
            "testing",
            "ndt_noise"
        ).takeIf { it.isNotEmpty() }?.split("#")?.firstOrNull() ?: getCheckItemValue(
            "noise_measurement",
            "pointA_result"
        )).toDoubleOrNull() ?: 100.0) < 85.0,
        isEmergencyStopFunctional = getCheckItemStatus(
            "testing",
            "func_safetyEmergencyStop"
        ) || getCheckItemValue("testing", "safety_emergencyStop").contains("Berfungsi", true),
        isMachineFunctional = getCheckItemStatus(
            "testing",
            "func_functionTest"
        ) || getCheckItemValue("testing", "ndt_loadTest").contains("Berfungsi", true),
        isVibrationLevelCompliant = getCheckItemStatus(
            "testing",
            "func_vibrationTest"
        ) || getCheckItemStatus("visual_inspection", "mainPartsVibrationDamper"),
        ...
        isShaftRotationCompliant = when (inspection.subInspectionType) {
            ...
            SubInspectionType.Motor_Diesel -> getCheckItemValue(
                "testing",
                "ndt_shaftRpm"
            ).isNotBlank()
        ...
        isGroundingResistanceCompliant = getCheckItemStatus(
            "testing",
            "func_safetyGrounding"
        ) || getCheckItemValue("testing", "safety_grounding").isNotBlank(),
        isNdtWeldTestOk = getCheckItemStatus(
            "testing",
            "func_weldJointTest"
        ) || getCheckItemValue("testing", "ndt_weldJoint").contains("Baik", true),
        isVoltageBetweenPhasesNormal = getCheckItemValue(
            "testing",
            "elec_panel_voltRS"
        ).isNotEmpty(),
        isPhaseLoadBalanced = {
    ...
    return PtpBAPReport(
        ...
        inspectionDate = Utils.formatDateToIndonesian(
            inspection.createdAt ?: inspection.reportDate ?: ""
        ),
        generalData = generalData,
        technicalData = technicalData,
        testResults = PtpBAPTestResults(visualInspection = visualInspection, testing = testing)
...
fun PtpBAPReport.toBapRequest(
    ...
    return MachineBapRequest(
        ...
        createdAt = createdAt,
        extraId = extraId,
        generalData = MachineBapGeneralData(
            this.generalData.companyName,
            this.generalData.companyLocation,
            this.generalData.usageLocation,
            this.generalData.addressUsageLocation
        ),
        technicalData = MachineBapTechnicalData(
            this.technicalData.brandOrType,
            this.technicalData.manufacturer,
            this.technicalData.manufactureYear,
            this.technicalData.serialNumber,
            this.technicalData.capacityDescription,
            this.technicalData.driveMotorPowerKw,
            this.technicalData.specialSpecification,
            this.technicalData.dimensionsDescription,
            this.technicalData.rotationRpm,
            this.technicalData.frequencyHz,
            this.technicalData.currentAmperes,
            this.technicalData.machineWeightKg,
            this.technicalData.areSafetyFeaturesInstalled
        ),
        visualChecks = MachineBapVisualChecks(
            this.testResults.visualInspection.isMachineGoodCondition,
            this.testResults.visualInspection.areElectricalIndicatorsGood,
            this.testResults.visualInspection.personalProtectiveEquipment.isAparAvailable,
            this.testResults.visualInspection.personalProtectiveEquipment.isPpeAvailable,
            this.testResults.visualInspection.isGroundingInstalled,
            this.testResults.visualInspection.isBatteryGoodCondition,
            this.testResults.visualInspection.hasLubricationLeak,
            this.testResults.visualInspection.isFoundationGoodCondition,
            this.testResults.visualInspection.hasHydraulicLeak
        ),
        functionalTests = MachineBapFunctionalTests(
            this.testResults.testing.isLightingCompliant,
            this.testResults.testing.isNoiseLevelCompliant,
            this.testResults.testing.isEmergencyStopFunctional,
            this.testResults.testing.isMachineFunctional,
            this.testResults.testing.isVibrationLevelCompliant,
            this.testResults.testing.isInsulationResistanceOk,
            this.testResults.testing.isShaftRotationCompliant,
            this.testResults.testing.isGroundingResistanceCompliant,
            this.testResults.testing.isNdtWeldTestOk,
            this.testResults.testing.isVoltageBetweenPhasesNormal,
            this.testResults.testing.isPhaseLoadBalanced
        )
    )
}
 ...
 */
fun PtpBAPReport.toDieselBapRequest(
    laporanId: String,
    createdAt: String,
    extraId: Long
): DieselBapRequest {
    return DieselBapRequest(
        ...
        createdAt = createdAt,
        extraId = extraId,
        generalData = DieselBapGeneralData(
            this.generalData.companyName,
            this.generalData.companyLocation,
            this.generalData.usageLocation,
            this.generalData.addressUsageLocation
        ),
        technicalData = DieselBapTechnicalData(
            this.technicalData.brandOrType,
            this.technicalData.manufacturer,
            this.technicalData.manufactureYear,
            this.technicalData.serialNumber,
        ...
        visualChecks = DieselBapVisualChecks(
            this.testResults.visualInspection.isMachineGoodCondition,
            this.testResults.visualInspection.areElectricalIndicatorsGood,
            this.testResults.visualInspection.personalProtectiveEquipment.isAparAvailable,
            this.testResults.visualInspection.personalProtectiveEquipment.isPpeAvailable,
        ...
        functionalTests = DieselBapFunctionalTests(
            this.testResults.testing.isLightingCompliant,
            this.testResults.testing.isNoiseLevelCompliant,
            this.testResults.testing.isEmergencyStopFunctional,
            this.testResults.testing.isMachineFunctional,
