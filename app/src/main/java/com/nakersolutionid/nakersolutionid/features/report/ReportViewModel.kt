...
import com.nakersolutionid.nakersolutionid.features.report.elevator.CarSignageUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.CarUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.CounterweightGuideRailsAndBuffersUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.DrumsAndSheavesUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.ElectricalInstallationUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.ElevatorUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.FireServiceElevatorUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.GeneralDataUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.GovernorAndSafetyBrakeUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.HoistwayAndPitUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.InspectionAndTestingUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.MachineRoomAndMachineryUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.MachineRoomlessUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.ResultStatusUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.SeismicSensorUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.SuspensionRopesAndBeltsUiState
import com.nakersolutionid.nakersolutionid.features.report.elevator.TechnicalDocumentInspectionUiState
...
class ReportViewModel(private val reportUseCase: ReportUseCase) : ViewModel() {
    ...
    fun onSaveClick(selectedIndex: Int) {
        val currentState = _uiState.value
        when (selectedIndex) {
            0 -> {
                viewModelScope.launch {
                    reportUseCase.sendReport(currentState.toDomain()).collect { result ->
                        _uiState.update { it.copy(elevatorResult = result) }
                    }
                }
            }
            1 -> {
                _uiState.update { it.copy(eskalatorResult = Resource.Error("Not Implemented Yet")) }
            }
        }
    }

    fun onElevatorStateHandledSuccess() {
        _uiState.update { it.copy(elevatorResult = null) }
    }

    fun onElevatorStateHandledFailed() {
        _uiState.update { it.copy(elevatorResult = null) }
    }

    fun onEskalatorStateHandledSuccess() {
        _uiState.update { it.copy(eskalatorResult = null) }
    }

    fun onEskalatorStateHandledFailed() {
        _uiState.update { it.copy(eskalatorResult = null) }
    }
